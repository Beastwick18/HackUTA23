...
def create_game(image, labels, selected):
    ...
    room = requests.post("https://focus-sequencer-401321.uc.r.appspot.com/api/create_entry",data)
    print(room.content)
...
def waiting_loop(data, start):
    ...
    while(True):
        ...
        if start:
            new_r = requests.post("https://focus-sequencer-401321.uc.r.appspot.com/api/room_start", json=data)
            break
        time.sleep(1)
    return
...
if picture:
    ...
    r = requests.post("https://focus-sequencer-401321.uc.r.appspot.com/api/upload_image", data)
